%Performs first-order upwind finite difference method for u_t = -u_x. Input arguments are
...
function u = first_order_upwind(t_nodes,x_nodes,init_cond,left_bcs,right_bcs,t_steps)
...
%Make the Q matrix
Q = full(gallery('tridiag',x_nodes-2,lambda,1-lambda,0));
...
%Iterate the first-order upwind scheme, at each time step plotting u vs x
for i=1:t_steps
    F(1) = lambda*l_bcs(i);
    v = Q*v + F;
