...
function [l2_err_12,l2_err_24] = l2_richardson_325(h,alpha,beta,a,b,f,m)

syms x;

z1 = linspace(0,1,(1/h)+1);
[u_h,theta_h,h] = pw_linear_FEM_325(z1,alpha,beta,a,b,f,m);

z2 = linspace(0,1,(2/h)+1);
[u_2h,theta_2h,h2] = pw_linear_FEM_325(z2,alpha,beta,a,b,f,m);

z4 = linspace(0,1,(4/h)+1);
[u_4h,theta_4h,h4] = pw_linear_FEM_325(z4,alpha,beta,a,b,f,m);
...
for i=1:length(z2)-1
    ...
    for j=1:length(y)
        l2_err_12 = l2_err_12 + subs(((r1(c)*theta_h(c)+r1(c+1)*theta_h(c+1))-(r2(i)*theta_2h(i)+r2(i+1)*theta_2h(i+1)))^2,x,y(j))*w(j);
    end
end
...
for i=1:length(z4)-1
    ...
    for j=1:length(y)
        l2_err_24 = l2_err_24 + subs(((r2(c)*theta_2h(c)+r2(c+1)*theta_2h(c+1))-(r4(i)*theta_4h(i)+r4(i+1)*theta_4h(i+1)))^2,x,y(j))*w(j);
    end
end
