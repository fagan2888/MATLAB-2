%Modified lines 174-183 and 237-270 to get L^2 and H^1 errors, 
%as well as to modify the messages that were outputted
   ...
   if nargin>=10 && isfield(sol,'fnu')
     if method==1
        errsinf=[errsinf;toterr];
         ...
         %Next line is altered
        errsinf=[errsinf;L2NormErr1(T,sol.fnu,U,g)];
     end
   end
   if nargin>=10 && isfield(sol,'fnux') && isfield(sol,'fnuy')
      if method==0
         errsE=[errsE;toterr];
          ...
          %Next line is altered
         errsE=[errsE;sqrt((EnergyNormErr1(T,1,sol.fnux,sol.fnuy,U,g))^2 + (L2NormErr1(T,sol.fnu,U,g))^2)];
      end
   end
   if trace
...
if trace && length(N)>2
   ...
   if nargin>=10 && isfield(sol,'fnu') && isfield(sol,'fnux') && ...
                                        isfield(sol,'fnuy')
      %Next line is altered
      fprintf( '   N      H^1 Err.      L^2 Err.     C (L^2)\n' )
      ...
      C1=(N1.^(-0.5))\err1;
      %Next line is altered
      disp('Fitting errors to err=CN^q:')
      %Next line is altered
      disp(['L^2 norm: c=',num2str(exp(c(1))),', p=',num2str(c(2))])
      %Next line is altered
      disp(['H^1 norm: c=',num2str(exp(c1(1))),', p=',num2str(c1(2))])
