%Gockenbach code altered by Joshua Enxing where indicated
...
for i=1:N
   ...
   err1=sqrt(EnergyNormErr1(T,1,ux,uy,U,g)^2+err0^2);
   disp(['Computing the norms of the error: ',...
          num2str(etime(clock,t1)),' seconds'])
      
   %Next line is altered
   disp(['L2, H1: ',num2str(err0),', ',num2str(err1)])
   if i>1
...
%Next two lines are altered
nrm1=sqrt(EnergyNorm1(T,1,ux,uy)^2+nrm0^2);
disp(['L2, H1: ',num2str(nrm0),', ',num2str(nrm1)])
